...
function Navbar() {
  ...
  const displayLinks = useBreakpointValue({ base: "none", md: "block" });

  if (authDetails.isLoggedIn && !showName) {
    setTimeout(() => {
      setShowName(true);
    }, 2000);
  }

  if (authDetails.isLoggedIn && !showLogout) {
    setTimeout(() => {
      setShowLogout(true);
    }, 2000);
  }
  ...
  return (
    <div>
      <Box bg="#2D2D2D" width={{ base: '100%', md: '100%', lg: '100%' }}>
        <HStack spacing={{base:'1',md:"2",lg:'4'}} p={{base:'1',md:"2",lg:'3'}}>
          <Box>
            <Image
              cursor="pointer"
              onClick={() => navigate("/")}
          ...
          <Box>
            {links.map((ele, i) => (
              <Link className="hove" key={i} to={ele.to}>
                {ele.label}
              </Link>
            ))}
          </Box>
          ...
          <Box color="white" fontWeight="600">
            {showName &&
              ...
              `Hi, ${
                authDetails.firstname.charAt(0).toUpperCase() +
                authDetails.firstname.slice(1)
              }`}
          </Box>
        </HStack>
      </Box>
